<script>
  ...
  async function addReport() {
    ...
    try {
      const testData = { restaurant_id: 1, comment: reportInput, type: '테스트 타입', user: "soyoung" };
...
<form class="flex flex-col" on:submit|preventDefault={addReport}>
  <textarea bind:value={reportInput} placeholder="내용을 입력해주세요. (ex.메뉴정보가 틀립니다.)" rows="4" class="m-4 p-3 border-1" />
  <!-- Modal footer -->
  <div class="flex items-center p-4 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">
    <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">신고하기</button>
